/*
	...
	=> trim() : 좌우 공백 제거
		=> 원형 : String trim()
			=> 로그인 입력, 검색어 입력, 회원가입
			=> 유효성 검사
	=> equals() : 같은 문장니지 확인 (대소문자 구분)
		=> 원형 : boolean equals(String msg)
			=> 로그인 (아이디,비밀번호)
	=> replace() : 문자, 문자열 변경 => 데이터베이스
		오라클에선 -> ||(문자열 결합) , &(입력값)
				   => 이미지 (URL)
		=> 원형 : String replace(char c,char c1) // 문자 바꾸기 
							    ------ -------
							      old    new 
				 String replace(String s,String s1) // 문자열 바꾸기 
				 				-------- ----------
				 				  old      new 
	=> valueOf() : 모든 데이터형을 문자열로 변환
		=> 원형 : String valueOf(int a) => boolean b
		=> static : 자동 메모리 할당
		   String.valueOf() // 유일하게 클래스명으로 접근 다른 것들은 변수로 접근 
...
//		문자열_1 a=new 문자열_1();
//		// 클래스 저장
//		System.out.println(a); // 원시값 => 객체의 실제 주소값 출력 => 생략 가능
