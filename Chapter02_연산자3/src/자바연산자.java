 ...
 *   ---------------
 *    정수
 *     byte (8bit) => 0,1을 8개 채워서 숫자 한개 저장
 *          -128~127
 ...
 *      boolean bb;
 *      
 *    ---> 데이터 저장 ---> 사용자 요청에 맞게 데이터 가공 ---> 화면 출력
 *    ---> 프로그램 (반복)            |
 *                               연산자
 *    자바에서 제공하는 연산자
 *    -----------------
 ...
 *           double로 변경된다
 *           
 *           *** 주의점 => int 이하 데이터형은 연산시에 int로 변경된다
 *           char+byte = int
 *           char+char = int
 *           byte+byte = int
 *        
 ...
 *         <= 작거나 같다
 *         >= 크거나 같다
 *         *** 날짜,문자열은 비교할 수 없다 (자체 메소드)
 *       = 논리연산자 (&&,||)
 *         && : 직렬연산자 (조건 2개가 true 일 경우에만 true)
 *              (조건) && (조건)
 *              ----     ----- 둘 다 true 여야 true
 *              주로 사용처 : 기간 , 범위가 포함된 상태
 *         || : 병렬연산자 (조건 2개중에 1개 이상이 true 일 경우 true)
 *              (조건) || (조건)
 *              ----     ----
 *              주로 사용처 : 잘못된 입력
 *       = 대입연산자 (=,+=,-=)
...
import java.util.Scanner; //Scanner 클래스를 가지고 온다 (java.util < 폴더(패키지)명)
// C => include
// C# => using
// 자바스크립트 => import
public class 자바연산자 {
