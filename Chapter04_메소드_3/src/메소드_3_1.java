	...
	1. 메소드 구성요소 
	   = 리턴형 : 사용자 요청에 대한 처리 결과값 
	            결과값은 반드시 한개만 사용이 가능하다
	                        ----
	                          | 여러개인 경우 => 배열/클래스/컬렉션
	            => 예측해서 설정해야한다 (int일지, String일지, void일지)
	   = 매개변수 : 사용자가 요청한 값 
	              웹에서 마우스 클릭 , 엔터 , 버튼 , 입력
	                   ---------               ----
	   ...
	   = 메소드명 : 식별자 (변수)
	            => 소문자로 시작한다 ... etc
	   = return 문장 : 처리 결과값을 보내준다 , 메소드의 종료 
	     ---------- 맨마지막만 사용이 가능한 게 아니라 중간에서도 사용이 가능
     ex) 메소드()
	     {
	     	if()
	     	{
...
접근지정어------------------------------------
	   ...
	   protected : 상속받는 클래스까지만, 같은 폴더안에서
	   default : 같은 폴더안에서만
	   private : 자신의 클래스안에서만 => 은닉화
	옵션------------------------------------
	   static : 자동 메모리 저장 ==> new 쓸 필요 없다
	   abstract : 추상 메소드 ==> 선언만 (설계단에서 => 요구사항)
   ...
   리턴형------------------------------------
	      ...
	      X        X 
	      	...
	      	*** 한개의 클래스안에서 메소드명이 동일할 수 있다
	      					   -----------
	      					        |
	      					 매개변수의 갯수나 데이터형이 다르다
	      				  aaa(int a) [ add(int b) 는 같은 걸로 인식! ]
	      				  aaa(double d)
	...
	4. 메소드 호출 
	   --------
	   = 인스턴스 메소드 => new를 이용해서 메모리 저장 : (내용이 바뀌는 것들)
	     Scanner sc = new Scanner(System.in)
	     sc.nextInt()
	     String s="";
	     s.메소드
	   = 클래스 메소드 (static) = 공유 => 자동 저장 : (내용이 안바뀌는 것들)
	     Math.random()
...
//				if(arr[i]<arr[j]) // ASC, DESC 나눠서 할 때
//				{
//					int temp = arr[i];
//					arr[i] = arr[j];
//					arr[j] = temp;
