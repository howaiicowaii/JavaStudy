/*
 * ----25p 관리할 데이터를 저장하는 공간을 '변수'라고 한다
 * ------------------------- 한개만 저장이 가능하다
 *   변수 : 변수명 => 소문자로 시작한다
 *        식별자 (명명규칙)
 *        1. 알파벳이나 한글로 시작한다
 *           ---- 대소문자 구분
 *           Abc abc ABC => 전부 다 다른 변수
 *        2. 명칭의 갯수는 제한하지 않는다
 *           => 3 ~ 7글자로 압축
 *           a , b , c  => kor , math , eng
 *        3. 숫자는 사용이 가능하다
 ...
 *       float
 *          float f=10.5
 *                  ----- double 이기 떄문에 오류 발생 so 10.5F 로 해줘야함
 *          (10.5 는 double 이 default 값이기 때문에 뒤에 F를 붙여 float으로)
 *       double
 *          double d='A'  d=65.0
 *          double d=10   d=10.0
 *          double d=10.5F  d=10.5
 *          double d=10L  d=10.0  (더 큰거로 바뀌어서 저장됨)
 *          
 */
public class 자바변수 {
