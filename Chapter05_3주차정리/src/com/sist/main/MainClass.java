	...
	2. 클래스를 만드는 과정
	   1) 클래스 구성요소
 			   ...
 			   boolean b; => false ==> 한번을 메모리에 저장 
 			                   => 필요한 값으로 변경해서 사용 
 			                      -------
 			                       초기화 블록 / 생성자
 			 ==> 권장
 			 String name,address,phone... -> 이렇게 한줄에 쓰지 말고 위처럼 
 			 ***** 데이터(변수)는 반드시 외부에서 접근, 변경, 손실 => 은닉화 
 			       private ==> 모든 변수는 private 사용
	      	-------------------
	      	 생성자 => 접근이 가능 => public
	      	      => 무조건 생성하는 것이 아니다 
	          ...
	          2) void method()
	             {
	      	        return; => 생략할 경우 자동으로 추가 (void인 경우)
	      	     }
	      	  3) 자바의 모든 클래스는 Object 클래스 상속
	      	                     ---------------
	      	     class A extends Object 
	      	             -------------- => Object에서 지원하는 모든 메소드 사용 가능
	      	  ...
	      	  2) 생성자의 특징 
	      	                ...
	      	                | 오버로딩 (중복함수정의)
	      	                  ...
	      	                  1) 조건 
	      	                   ...
	      	                   = 리턴형은 관계없다
	      	                     ...
	      	                     void aaa(int b) ==> aaa(int) 위와 같은 메소드
	      	                   = 같은 클래스에서 제작
	      	                   = 생성자 , 멤버메소드에서도 적용
	      	    ...
	      	    3) 생성자의 사용처 
	      	      = 인스턴스 변수가 명시적인 초기화가 안되는 경우
	      	                    ----------- 파일읽기, DB 데이터 대입, 크롤링
	      	      = 시작과 동시에 처리 해야하는 명령문이 있는 경우 
	      	       ex) 자동 로그인, ID 등록, 윈도우 화면 디자인
	      	 ...
	      	 --------- 컴파일러에 의해 자동 호출
	      	  인스턴스 블록 => 인스턴스 블록보단 생성자를 주로 사용
	      	   ...
	      	   A aa=new A(); ==> aa.a => 이렇게 호출 안하고 
	      	   A.a => static 블록은 이렇게 호출 가능 
	      	   ...
	      	   *** 주의점 
	      	   ...
	      	   class A
	      	   {
	      	      ...
	      	      static 
	      	      {
	      	         => 객체 생성후에 사용해야 인스턴스 사용 가능
	      	         a=1000; ==> Error 
	      	         b=2000; // static 블록은 static 변수인 경우에만 사용이 가능 
	      	      }
	      	      ----------------
	      	   }
	      	...
	      	-------------------
	      	 ...
	      	 형식)
	      	    -멤버메소드 
	      	 	...
	      	 	s.substring()
	      	 	s.trim()
	      	 	s.replace() => 멤버메소드  (인스턴스는 반드시 객체 생성 필요)
	      	 	----------------
	      	 	...
	      	 	메소드 (접근지정어는 public) => 다른 클래스와 연결할 때 사용
	      	 	 = 리턴형 
	      	 	       ...
	      	 	       => 소프트웨어 이행 : 영문을 사용한다
	      	 	          -----------
	      	 	           서버는 리눅스 => 차세대 운영체제 (우분투)
	      	 	                      => 우분투 (AWS)
	      	 	           개발 : 윈도우/맥 => 호스팅 : 리눅스  
	      	 	           => 프로젝트 : Git(원격) (윈도우 <=> 맥 경로명 주의해야한다)                    
	      	 	 = 매개변수 
	      	 	   사용자 요청 값 => 로그인(id,pwd,check)
	      	 	   => 사용자의 입력, 마우스 클릭, 키보드 이용 => 매개변수로 처리
	      	 	   => 매개변수의 갯수는 제한이 없지만 => 가급적 적게 사용한다
	      	 	                                 ------------
	      	 	                                  ㄴ 3개 
	      	 	                                  3개이상이면 모아서 전송
	      	 	                                   ( 배열 / 클래스 ) 
				   => 매개변수는 메모리 할당 => 메소드 호출시마다 메모리 생성
				   => 매개변수는 지역변수의 일종 => 메소드 종료시 자동으로 사라진다
				      => 자동 해제 (메모리에서 자동 처리)
				      => 초기값을 주는 경우 
				         메소드 호출시에 초기화를 해준다
				      ex) 
	      ...
	      *** 코드영역 
	      class
	      {
	         선언만 가능 : 변수 선언 , 메소드 선언 만 가능
	                    => 메소드 호출 , 연산처리 , 제어문 사용 불가능
	      }
	      method
	      {
	         선언(지역변수), 메소드 호출, 연산처리, 제어문 가능 => 구현 
	         => 사용자 요청 => 처리 => 결과값 전송 
	      }
	      메소드 => 생성자 , 초기화블록 (구현 가능)
	      *** 사용자 정의 로 만든 메소드,생성자
	          => 반드시 호출해서 사용해야 한다 (자동 호출이 없다)
	          => 초기화 블록 (자동 호출된다) => 컴파일러에 의해 한번만 수행
	      ==> CRUD (읽기 쓰기 수정 삭제)
	          회원가입 / 회원 정보 / 회원 수정 / 회원 탈퇴
	          => 면접 : 게시판 만들 수 있나? => CRUD 가능한가?
	   2) 메모리 저장 
	   3) 활용 (호출)
	3. 클래스의 종류 (5가지)
	   1) 일반 클래스 
