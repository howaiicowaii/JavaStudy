		...
		*** 자바는 모든 코딩 (변수,메소드...) => 클래서 안에서만 
			...
			ex)
				class ClassName
				{
					...
					void aaa()
					{
						static int b; => 오류 , 클래스 블록에서 사용 가능
					}
				}
...
*/
// package import 만이 class 위에 쓰일 수 있다.
class Sawon{
	int sabun; 
	String name;
	String dept,job,loc;
	// Sawon(){} ==> Sawon s=new Sawon() // 없는 경우 생성자 자동 추가
	Sawon(){ // 생성자 
		sabun=1;
		name="홍길동"; // 파란색 => 멤버변수
		dept="개발부";
...
public class MainClass {
	...
	public static void main(String[] args) {
		...
		Sawon sa1=new Sawon(); // Sawon 생성자에 매개변수없어서 값이 다 같음 
		System.out.println("사번:"+sa.sabun);
		System.out.println("이름:"+sa.name);
		
		Sawon sa2=new Sawon(2,"박문수","영업부","과장","경기");
		System.out.println("sa2="+sa2);
		System.out.println("사번:"+sa2.sabun); // 0 밤색 => 지역변수
		System.out.println("부서:"+sa2.dept); // null 밤색 => 지역변수
