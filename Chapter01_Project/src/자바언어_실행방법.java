 ...
 *                HTML / CSS 
 *       = 자바 보안 좋은 이유 => 두번을 검색하기 때문에 (소스 검증)
 *         => 보안이 뛰어나다 : => 인증 시스템
 *         => 동원 증권 : 자바 => 시스템 구축 
 *         
 *                 자바소스
 ...
 *                대소문자를 구분한다
 *                => 언어 (대소문자를 구분한다)
 *            문법사항은 아니고 약속 (자바 개발자들끼리의)
 *         => 클래스명은 첫자를 대문자, 새로운 단어 결합시 첫자를 대문자로
 *            -------------------------- 헝거리언식 표현법
 *         => 변수, 메소드 => 소문자
 *            상수 => 전체를 대문자 
 *         => {} 일치 (열고 닫기)
 ...
 *         => Ctrl + Shift + \ => 주석 풀기
 *         => 화면 출력
 *            sysout 입력 후  Ctrl + space 하면 자동으로 완성
 *                          ------------
 *    잘 모를 때 . 찍고 Ctrl + Space 하면 나옴
 *    
 *         자바의 구성도
 *         class A
 *         { 
