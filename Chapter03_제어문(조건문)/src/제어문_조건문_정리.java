 ...
 *      
 *      22p => 들여쓰기
 *      23p => 주석 : // (한줄 주석) => 번역이 안되는 부분  => 소스 설명
 *      ------------------------------------------------------
 *      약속
 *       => 1. 클래스명은 영문 대소문자로 시작한다
 *          2. 변수는 소문자로 시작한다 (새로운 단어 나오면 대문자로 시작)
 ...
 *        
 *        true / false 저장 공간
 *        		boolean : 1byte => ture / false 만 저장이 가능 
 *        -------------------------------------- 30p 
 *        
 *        변수의 명명법 (메소드, 클래스, 인타페이스... 적용) : 식별자
 *        ----------
 *        1) 알파벳이나 한글로 시작한다 
 *        	 ---- 대소문자를 구분 
 ...
 *          대입연산자 (=, +=, -=)
 *        삼항연산자 
 *        	(조건) ? 값1 : 값2 ==> if~else 문장으로 대체 가능
 *        ---------------------------------------------
 *        54p (3장) 
 *        ----
 *         자바에서 제공하는 제어문
 *         조건문 
 *         	= 단일조건문 (중복값까지 다) : 독립적으로 사용 => 해당되는 조건이 많은 경우 
 *         		형식)
 *         			if(조건문)
 *         			{
 *         				조건문이 true일 경우에만 처리
 *         			}
 *         			=> false면 건너뛴다 
 *         	= 선택조건문 (2개 중) : true와 false를 나눠서 처리 
 *         		형식)
 ...
 *         			}
 *         조건문 54p
 *         	= 다중조건문 (1개만 출력) : 조건문 여러개를 나열 => 해당 조건 1개만 수행 
 *         	  형식)
 *         			if(조건문)
 *         			{
 *         				조건이 true => 문장 수행 후 종료 
