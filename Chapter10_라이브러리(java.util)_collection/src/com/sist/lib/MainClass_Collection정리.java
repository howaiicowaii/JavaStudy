       ...
       List : ArrayList
         ...
         => 주요 메소드 
            ...
            8) 두개의 List 가 있는 경우 
               ...
               => 합집합 => addAll()
               => 차집합 => removeAll()
            *** Arrays.asList() => 배열 => ArrayList 로 변경 
       Set : HashSet
         => 순서가 없다 
         => 중복을 허용하지 않는다 
            ** null 값을 추가할 수 있다 (한번만 가능하다)
         => 장르별 , 부서별 , 상품별 
         => List 에서 중복 데이터를 제거할 때 
         => 주요메소드 
            1) 추가 : add()
            2) 삭제 : remove(Object)
            3) 존재여부 : isEmpty()
            4) => 일반 for 는 사용할 수 없다 => for-each (인덱스가 없기 때문에)
            5) 갯수 : size()
       Map : HashMap
...
public class MainClass_Collection정리 {
	...
	public static void main(String[] args) {
		...
		try
		{
			...
			prop.load(new FileInputStream("/Users/maclovin/eclipse-workspace/Chapter10_라이브러리(java.util)_collection/src/com/sist/lib/student.properties"));
