/*
 *   1. 실행 순서
 *      => 자바 파일명과 클래스명 동일해야함 (대소문자 구분)
 *              컴퓨터가 인식하는 언어
 *                javac(컴파일) => 0,1 (2진법)  java(인터프리터)
 *      A.java ================> A.class =================> 실행
 *      ------                   바이너리코드
 *        |                    모든 운영체제에 호환
 *      원시소스(프로그래머)
 ...
 *      변수 : 소문자로 시작 => 새로운 단어나오면 첫자 대문자로
 *            fileName , file_name
 *      상수 : 모든 알파벳 대문자로
 *            MAX , MIN...
 *      클래스, 인터페이스 : 첫자는 대문자로 => MainClass
 *      메소드 : 변수와 동일
 *      
 *   *** 프로그램 (데이터 관리)
 *       데이터 저장
 ...
 *        변수명 설정
 *        --------
 *        1. 알파벳 시작 (대소문자 구분), 한글 (웹상에서는 종종 깨짐)
 *        2. 숫자 사용 가능 (앞에 사용 불가능)
 *        3. 키워드는 사용할 수 없다
 *        4. 공백없이 사용
 *        5. 특수문자 ($ , _) 사용 가능
 *        6. 기타사항 => 변수명 압축(3~7) => i,j는 되도록 쓰지 않기
 *    ---------------------------------------------------
 *    변수 선언 방식
 *    ->데이터형 변수명;
 *    변수의 초기화
 ...
 *    변수의 종류 ===> 지역변수 = {}안에서 사용하는 변수 << 이것만 배울 것
 *    
 ...
 *          
 *          왼쪽 <= 오른쪽
 *      ex) int a=10.5; ==> 10.5는 double로 8byte, int는 4byte라 오류 발생
 *      --------------------------------------------------------------
 *   
 */
// 3개 정수 (1~100) => 총합 , 평균
public class 산술연산자 {
	...
	public static void main(String[] args) {
		...
		int a=(int)(Math.random()*100)+1;  // => 1~100 (+1 해줘서)
